...
qemu_args=-M q35  -bios /usr/share/edk2/x64/OVMF.4m.fd -drive format=raw,file=$(bootimg_uefi),if=none,id=boot -device virtio-blk,serial=fee1dead,drive=boot -serial stdio -m 2048 -no-reboot
#qemu_args=-M q35 -bios /usr/share/edk2/x64/OVMF.4m.fd -drive format=raw,file=$(bootimg_uefi),if=none,id=nvme -device nvme,serial=fee1dead,drive=nvme -serial stdio -m 2048 -no-reboot -enable-kvm
