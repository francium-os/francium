...
impl Mailbox {
	pub fn new(peripheral_base: usize) -> Mailbox {
		let mailbox_virt = syscalls::map_device_memory(peripheral_base + 0xb000, 0, 0x1000, MapType::Device, PagePermission::USER_READ_WRITE).unwrap() + 0x880;
		Mailbox {
...
impl MailboxMessage<'_> {
	...
	pub fn send(&mut self, requests: &[MailboxRequest]) {
		...
		for i in (0..self.offset).step_by(4) {
			print!("0x{:x}, ", u32::from_le_bytes(self.buffer[i..i+4].try_into().unwrap()));
		}
		println!();
	}
...
impl MailboxAdapter {
	...
	fn send_mailbox_messages(&mut self, requests: &[MailboxRequest]) -> Vec<MailboxReply> 
	{
		...
		for i in (0..4096).step_by(64) {
			unsafe { clear_cache_for_address((msg.buffer.as_ptr() as usize) + i); }
		}
