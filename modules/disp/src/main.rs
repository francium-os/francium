    ...
    for y in (0..16).step_by(2) {
        ...
        for x in (0..16).step_by(2) {
            if (row & (1<<(x/2))) != 0 {
                ...
                fb[(xx+x+1)+(yy+y)*pitch] = 0xffffffff;
                fb[(xx+x)+(yy+y+1)*pitch] = 0xffffffff;
                fb[(xx+x+1)+(yy+y+1)*pitch] = 0xffffffff;
            }
        }
    }
}
...
fn main() {
    ...
    let (_fb_width, _fb_height, fb_stride, fb) = match platform {
        Platform::Virt | Platform::Pc => {
            if let Some(mut bochs) = bochs::BochsAdapter::new() {
                bochs.set_mode(1024, 768);
                (1024, 768, 1024, bochs.get_framebuffer())
            } else if let Some(platform) = platform::PlatformFramebuffer::new() {
                (platform.info.width, platform.info.height, platform.info.stride, platform.get_framebuffer())
            } else {
    ...
    for y in 0..splash_y {
        for x in 0..splash_x {
            ...
            fb[x + y*fb_stride] = (splash_rgb[splash_offset] as u32) | (splash_rgb[splash_offset+1] as u32) << 8 | (splash_rgb[splash_offset + 2] as u32) << 16;
        }
    }

    // print platform
    let mut off = print_string(fb, fb_stride, 0, 600, "Hello from platform '");
    off += print_string(fb, fb_stride, off, 600, &platform_name);
    off += print_string(fb, fb_stride, off, 600, "'");
