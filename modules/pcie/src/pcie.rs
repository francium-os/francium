...
fn get_function_header(block: usize, bus: u8, device: u8, function: u8) -> &'static mut ConfigurationSpaceHeader {
    let offset: usize = ((bus as usize) << 20) + ((device as usize) << 15) + ((function as usize) << 12);
    unsafe {
        ((block + offset) as *mut ConfigurationSpaceHeader).as_mut().unwrap()
    }
}

fn get_function_type0(block: usize, bus: u8, device: u8, function: u8) -> &'static mut ConfigurationSpaceType0 {
    let offset: usize = ((bus as usize) << 20) + ((device as usize) << 15) + ((function as usize) << 12);
    unsafe {
        ((block + offset) as *mut ConfigurationSpaceType0).as_mut().unwrap()
    }
}
    ...
    fn discover_function(&mut self, block: usize, bus_num: u8, device_num: u8, function_num: u8) -> (bool /* valid */, bool /* is multifunction */) {
        ...
        if function.vendor_id != 0xffff {
            ...
            match header_type {
                /* device */
                0 => {
                    let function_type0 = get_function_type0(block, bus_num, device_num, function_num);
                    for bar in function_type0.bars {
                        println!("{:08x}: Bar type: {}, location: 0b{:02b}, prefetchable: {}", bar, bar&1, (bar&(3<<1)) >> 1, (bar&(1<<3)) >> 3);
                    }
                    self.functions.push(PCIFunction { num: function_num, inner: function_type0 });
                }
                /* pci bridge */
                1 => {
...
pub fn scan_via_device_tree(dt_addr: usize) -> Vec<PCIBus> {
    ...
    let dt_header_virt = syscalls::map_device_memory(dt_addr, 0, 0x1000, PagePermission::USER_READ_WRITE).unwrap();
    let dt_len = unsafe { DevTree::read_totalsize(std::slice::from_raw_parts(dt_header_virt as *const u8, 0x1000)) }.unwrap();

    let dt_virt = syscalls::map_device_memory(dt_addr, 0, align_up(dt_len as usize, 0x1000), PagePermission::USER_READ_WRITE).unwrap();
    let fdt = unsafe {
        DevTree::from_raw_pointer(dt_virt as *const u8).unwrap()
    };
        ...
        while let Some(prop) = prop_iter.next() {
            ...
            if name == "ranges" {
                ...
                for i in (0..num_ranges*7).step_by(7) {
                    ...
                    let pci_addr = prop.u32(i+2).unwrap() as u64 | (prop.u32(i+1).unwrap() as u64) << 32;
                    let host_addr = prop.u32(i+4).unwrap() as u64 | (prop.u32(i+3).unwrap() as u64) << 32;
                    let host_size = prop.u32(i+6).unwrap() as u64 | (prop.u32(i+5).unwrap() as u64) << 32;
                    println!("pci: ({:08x} {:x}) host ({:x}, {:x})", pci_hi, pci_addr, host_addr, host_size);
                }
            } else if name == "reg" {
                ...
                let ecam_virt = syscalls::map_device_memory(ecam_addr as usize, 0, ecam_size as usize, PagePermission::USER_READ_WRITE).unwrap();
